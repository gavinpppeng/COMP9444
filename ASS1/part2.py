...
class LinearModel:
    ...
    def forward(self, inputs):
        ...
        """
        return_sum = self.weights[self.num_inputs]
        for i in range(self.num_inputs):
            return_sum += self.weights[i] * inputs[i]
        return self.activation(return_sum)
    ...
    def backward(self, inputs, diff):
        ...
        """
        for i in range(self.num_inputs):
            self.weights[i] = self.weights[i] + inputs[i]*self.lr * diff
        self.weights[self.num_inputs] = self.weights[self.num_inputs] + self.lr * diff
